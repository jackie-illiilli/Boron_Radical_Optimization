...
def xtb_to_mol(mol, atoms, positions, conf_limit):
    """Update atom coordinates in mol molecule, delete existing conformations, add new ones.

    Args:
        mol (Chem.Mol): Original mol molecule
        atoms (list): List of element symbols
        positions (__iterable__): Multiple atom coordinates (m*n*3)
...
def xtb_update_mol(mol, xtbfile, conf_limit = 3, rmsd_limit=1.5):
    """Read conformations from xtb output file (.xyz), update based on rmsd and number limits.

    Args:
        mol (Chem.Mol): 
        xtbfile (str): xtb output file (.xyz)
        conf_limit: Limit to read only first conf_limit coordinates
...
def xtb_write_xyz(mol, atom_list=None, position_list=None, xtb_dir='xtb_process/', smiles_name='test', dist_rest=None):
    """
    Generate xtb optimization files for each conformation in a separate folder.
...
def write_xtb_pbs(pbs_path, charge, que="gamma", root_dir='charg_0', uhf=0):
    """Script generator for xtb, suitable for PBS queue in supercomputers.

    Args:
        pbs_path (_type_): Script file path
...
def xtb_main_2(smiles_names, smileses, restrict = None, dir_path='xtb_process', que="gamma", charges = None, uhfs = None):
    """ !! Main Process
    Suitable for tasks with different charges and unpaired electrons.
    Args:
        smiles_names: Identifier name for each smiles
        smileses (_type_): smiles or mols
        dir_path (str, optional): root dir saved charge files. Defaults to 'xtb_process'.
        que (str, optional): Supercomputing queue. Defaults to "gamma".
        charges: Charge list
        uhfs: Unpaired electron list
    """    
...
def xtb_main(smiles_names, smileses, restrict = None, dir_path='xtb_process', que="gamma", core=1, uhf=0):
    """ !! Main Process
    Categorize all molecules by charge and submit to multiple nodes.
    suball is the main submission script.

    Args:
        smiles_names: Identifier file name for each smiles
        smileses (_type_): smiles or mols
        dir_path (str, optional): root dir saved charge files. Defaults to 'xtb_process'.
        que (str, optional): Supercomputing queue. Defaults to "gamma".
        core(int): Number of separate node submissions.
...
def check_xtb_normal(root_dir):
    """Check if all Xtb files in all subdirectories computed successfully
...
def read_xyz(file_dir):
    """Extract all atom coordinates from .xyz file
    Args:
...
def xtb_is_success(xtb_dir):
    """Check if single xtb task is successful
...
def after_xtb(mol, xtb_dir="xtb_process", save_dir="xtb_result", conf_limit=3, rmsd_limit=1.5, xtb_title=None, method="opt freq b3lyp/6-31g* em=gd3bj g09def", SpinMultiplicity=None, charge=None):
    """For a given mol molecule, find the corresponding task and generate initial Gaussian files based on limits.

    Args:
        mol (Chem.Mol): mol
        xtb_dir (str, optional): Xtb root task directory. Defaults to "xtb_process".
        save_dir (str, optional): Gaussian input save dir. Defaults to "xtb_result".
        conf_limit (int, optional): Quantity limits. Defaults to 3.
        rmsd_limit (float, optional): RMSD limits. Defaults to 1.5.
        xtb_title (_type_, optional): Gaussian input files title. Defaults to None.
...
def shift_to_sugan(target_file, quene_id = 1,chrg=0, uhf=0):
    """Adjust submission script for Sugon supercomputer format.

    Args:
        target_file (_type_): Root directory of Xtb
        quene_id (int, optional): Queue ID. Defaults to 1.
...
def shift_to_sugan_2(target_file, quene_id = 1):
    ...
    Args:
        target_file (_type_): Xtb files root directory
        quene_id (int, optional): Queue ID. Defaults to 1.
    """    
    pbs_files = glob.glob(target_file + '/*.pbs')
    for pbs_file in pbs_files:
...
def shift_to_parra(target_file,chrg=0, uhf=0):
    """Adjust submission script for parallel cloud supercomputers.

    Args:
        target_file (_type_): Xtb root directory
        quene_id (int, optional): Queue ID. Defaults to 1.
        chrg (int, optional): Charge. Defaults to 0.
        uhf (int, optional): Unpaired electrons. Defaults to 0.
    """    
